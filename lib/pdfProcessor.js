...
export class PDFProcessor {
  ...
  async getDocumentsBySource(sourceFilter) {
    try {
      // Load stored vectors and metadata
      const vectors = JSON.parse(await fs.promises.readFile(this.vectorStoreFile, 'utf8'));
      const metadata = JSON.parse(await fs.promises.readFile(this.metadataFile, 'utf8'));
      ...
      metadata.forEach((meta, index) => {
        if (meta.source === sourceFilter) {
          filteredIndices.push(index);
          filteredDocuments.push(vectors[index].text);
          filteredMetadata.push(meta);
        }
      });
      
      console.log(`Found ${filteredDocuments.length} documents with source: ${sourceFilter}`);
  ...
  async processFile(fileBuffer, fileName, fileType, tempFilePath) {
    let text = '';
    
    try {
      if (fileType === 'application/pdf') {
        text = await this.extractTextFromPDF(fileBuffer, tempFilePath);
  ...
  async extractTextFromPDF(buffer, tempFilePath) {
    try {
      console.log('Extracting text from PDF using pdf-parse-fork...');
      
      // Use pdf-parse-fork to extract text directly from buffer
